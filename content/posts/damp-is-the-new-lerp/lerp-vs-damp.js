...
const sketch = () => {
  ...
  function drawLerp({ context, width, height, playhead }) {
    ...
    const start = [width * 0.25, height * 0.25];
    const end = [width * 0.75, height * 0.25];
  ...
  function drawDamp({ context, width, height, playhead }) {
    // Chosoe size of circle & stroke
    const lineWidth = 4;
    const radius = 20;

    // Choose a start and end point somewhere in our canvas
    const start = [width * 0.25, height * 0.75];
    const end = [width * 0.75, height * 0.75];
    ...
    // Interpolate the x dimension from start X to end X, using t
    const x = damp(start[0], end[0], 20, t);

    // Now interpolate the y dimension
    const y = damp(start[1], end[1], 20, t);
