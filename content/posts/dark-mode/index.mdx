...
The best way to avoid _the flash_ is to rely on CSS rather than JavaScript. The challenge is that I am using Styled-Components which is a CSS-in-JS library and rely's on the JS bundle to load and parse before the CSS can be appropriately updated. The Theme UI team came up with an interesting solution.

The first step is to convert the colour modes from hard-coded hex values to to CSS Custom Properties.
