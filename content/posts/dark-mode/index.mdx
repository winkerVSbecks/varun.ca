...
To allow the user to manually select the colour-mode we need to connect the `mode` state and updater to the toggle button. React's context API works perfectly for sharing this information. The `ColorModeContext` provides `{ mode, setColorMode }` which is then used accessed by the [CycleMode](https://github.com/winkerVSbecks/varun.ca/blob/master/src/components/cycle-mode.js) button.
