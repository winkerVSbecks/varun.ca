...
To allow the user to manually select the colour-mode we need to connect the `mode` state and updater to the toggle button. React's context API works perfectly for this. The `ColorModeContext` provides `{ mode, setColorMode }` which is then used by the [CycleMode](https://github.com/winkerVSbecks/varun.ca/blob/master/src/components/cycle-mode.js) button.
...
The generated HTML and CSS uses the default colour mode. When the user loads the page, it renders with that default colour mode. Once the JS loads, it updates the CSS to match the user's preferred choice. This causes the page to flash between the two styles. Not great user experience!

The best way to avoid _the flash_ is to rely on CSS rather than JavaScript. The challenge here is that I am using Styled-Components which is a CSS-in-JS library and rely's on the JS bundle to load and parse before the CSS can be appropriately updated. The Theme UI team came up with an interesting solution.

First, convert the colour modes from hard-coded hex values to to CSS Custom Properties.
Then generate global CSS that controls the value of these CSS Custom Properties using scoped class names, for example: `.varun-ca-dark` and `.varun-ca-light` ([color-mode-styles.js](https://github.com/winkerVSbecks/varun.ca/blob/master/src/theme/color-mode-styles.js)).

<JsToCssVars />

Lastly, inject a tiny script in your HTML to check local-storage for user's preferrence and set the appropriate class name to the body tag ([use-color-mode.js](https://github.com/winkerVSbecks/varun.ca/blob/master/src/use-color-mode.js)).
...
We are still using Styled-Components for theming. However, the values point to CSS custom properties and we can switch the theme by changing the class on the `body` element. Since, all the variable values are in global CSS and the above script runs as soon as the page loads, the user no longer sees a flash. The only time the user sees a flash is the first time they visit your site and they have enabled dark mode on their OS.

Gatsby does a lot of work to ensure that your site is performant. One of those things is using React SSR to generate a static site. Which unfortunately means that we have to do a bit more work to implement dark mode, but the trade-off is worth it and I got to learn a lot. That said, if you use Theme UI then you get all of this for free.

The one key takeaway from this whole process is that adding dark-mode means that you are creating more work for your future self. Each feature you add to your site will have to support it.
