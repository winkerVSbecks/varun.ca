...
preview: ../assets/chillwave-alt.gif
date: 2015-11-23
---

<CodePen
  hash="EVJGVj"
  title="This Also Chillwave"
  height="300"
  defaultTab="result"
/>
...
![chillwave](../assets/chillwave.gif)

I absolutely love this! First thing I did was _right click & inspect element_ and was expecting to find an SVG with SMIL animation baked in. To my surprise it turned out to be a [sprite](http://thisalso.com../assets/global/chillwave.png) based animation with 24 frames. So, let’s recreate this with only SVG.
...
![wave path](../assets/wave-path.jpg)
...
- Then we begin the curve `c` and add the first control point. This is using relative coordinates so, the _y coordinate_ is simply `0`. The _x coordinate_ is `m` times the amplitude; where `m = 0.512286623256592433`. The value of `m` is [chosen](http://stackoverflow.com/a/13935397/1365008) such that it approximately creates a sine wave.

```js
['M', 0, 0.625 * h, 'c', 0.25 * h * m, 0];
```

- Then we add the second control point. For the _y coordinate_ we have to go up by one amplitude and up means negative in SVG. Therefore, `-0.25 * h`. To calculate the _x coordinate_ we go all the way to the end `0.25 * h` and come back by `0.25 * h * m`.
...
<CodePen
  hash="KdYLJm"
  title="SVG wave – add the rounded ends"
  height="300"
  defaultTab="result"
/>
...
<CodePen
  hash="JYVqVe"
  title="Chillwave – Move & Wiggle the Wave"
  height="300"
  defaultTab="result"
/>
