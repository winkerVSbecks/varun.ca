...
- **Optional UI —** an iframe that renders the UI associated with your plugin. Inputs, sliders and buttons.
...
- **With UI & browser APIs —** Allows users to interact with your plugin on the canvas. Provides access to the full set of browser APIs like `<canvas>` and network.
...
The ID property will be assigned to you by Figma, through the _Create new Plugin_ feature.
...
At this point, the UI looks pretty raw, just default browser styling. Unfortunately, Figma doesn't offer any out of the box solution. There is no official component library to make your plugin UI match that of Figma. The closest thing we have is Thomas Lowry's [figma-plugin-ds](https://github.com/thomas-lowry/figma-plugin-ds) or [figma-plugin-ds-svelte](https://github.com/thomas-lowry/figma-plugin-ds-svelte) or [figma-ui-components](https://github.com/lessmess-dev/figma-ui-components) by lessmess.
...
To use React, initialize with the plugin with `-react` flag. Here's the full [source-code](https://github.com/winkerVSbecks/figma-plugin-example-confetti) for the React & figma-plugin-ds version of the plugin. If you are new to React, I highly recommend [React for Web Designers](https://www.framer.com/blog/posts/react-for-web-designers). The ID property will be assigned to you by Figma.
...
Let's look at an example, Pack. It is a plugin that fills a frame using the [Circle packing](https://generativeartistry.com/tutorials/circle-packing) effect. The user can choose between different fill shapes.
