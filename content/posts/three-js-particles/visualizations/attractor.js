...
export function createAttractor(length, simulation, radius, timeStep) {
  const positions = [];
...
export function updateAttractor(
  ...
  const normalizedPosition = mapToSphere
    ? currentPosition.clone().normalize().multiplyScalar(scale)
    : currentPosition.clone().multiplyScalar(scale);
