...
<SourceCard
  name="CDD Visualization Storybook"
...
Now that we have the basics down, let’s go back to the CDD visualization we’re building. The high-level elements of this scene are:
...
The CDD visualization has four states and the position, scale and colour of the meshes animate between each step. We can track all that info in an array and use component state for cycling between each step.
